...
// Fetch moisture for device index
int16_t DallasTemperature::getMoisByIndex(uint8_t deviceIndex){
    ...
    return getMois((uint8_t*)deviceAddress);

}

// returns moisture in 12bit form from sht11? on an OPI one wire temp cable
int16_t DallasTemperature::getMois(const uint8_t* deviceAddress){
    uint8_t r0 = 0;
    int16_t output = 0;
    //if device is correct type send commands to read Relative Humidity
    //if device is not correct type return 0
    if (deviceAddress[0] == DS28EA00MODEL){
        _wire->reset();
        _wire->reset();
        _wire->select(deviceAddress);
        _wire->write(0xA5, parasite);
        _wire->write(0x01, parasite);
        _wire->write(0xFE, parasite);
        _wire->read();
        _wire->read();
        _wire->write(0x03, parasite);
        _wire->write(0xFC, parasite);
        _wire->read();
        _wire->read();
        _wire->write(0x02, parasite);
        _wire->write(0xFD, parasite);
        _wire->read();
        _wire->read();
        _wire->write(0xF0, parasite);
        _wire->write(0x0F, parasite);
        _wire->read();
        _wire->read();
        _wire->write(0x02, parasite);
        _wire->write(0xFD, parasite);
        ...
        if(r0==15){
            r0=1;
            output = output + r0*2*2*2*2*2*2*2*2*2*2*2;
        } 
        _wire->select(deviceAddress);
        _wire->write(0xA5, parasite);
        _wire->write(0x01, parasite);
        ...
        if(r0==15){
            r0=1;
            output = output + r0*2*2*2*2*2*2*2*2*2*2;
        }   
        _wire->select(deviceAddress);
        _wire->write(0xA5, parasite);
        _wire->write(0x01, parasite);
        ...
        if(r0==15){
            r0=1;
            output = output + r0*2*2*2*2*2*2*2*2*2;
        }   
        _wire->select(deviceAddress);
        _wire->write(0xA5, parasite);
        _wire->write(0x01, parasite);
        ...
        if(r0==15){
            r0=1;
            output = output + r0*2*2*2*2*2*2*2*2;
        }   
        _wire->select(deviceAddress);
        _wire->write(0xA5, parasite);
        _wire->write(0x01, parasite);
        ...
        if(r0==15){
            r0=1;
            output = output + r0*2*2*2*2*2*2*2;
        }   
        _wire->select(deviceAddress);
        _wire->write(0xA5, parasite);
        _wire->write(0x01, parasite);
